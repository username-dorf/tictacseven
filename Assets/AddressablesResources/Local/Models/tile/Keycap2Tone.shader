Shader "URP/Keycap2ToneWaveRounded_v3_5"
{
    Properties
    {
        ...
        _DigitScale("Digit Scale", Range(0.5,2)) = 1
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            CBUFFER_START(UnityPerMaterial)
                ...
                float4 _DigitTint;
                float  _DigitOpacity, _DigitInset, _DigitCorner, _DigitSoft, _DigitScale, _DigitOffsetX, _DigitOffsetY, _DigitRotation;
            CBUFFER_END

            TEXTURE2D(_WaveTex);  SAMPLER(sampler_WaveTex);
            TEXTURE2D(_DigitTex); SAMPLER(sampler_DigitTex);
            ...
            float4 frag(Varyings i) : SV_Target
            {
                ...
                float m = (_EnableWave > 0.5)
                    ? ((_UseWaveTex > 0.5) ? waveMask_tex(i.uv) : waveMask_proc(i.uv, radians(_WaveAngle)))
                    : 1.0;

                float3 topTwo = lerp(_TopColorB.rgb, _TopColorA.rgb, m);

                float m0 = maskRounded(p, _CornerRadius, _Margin, _Feather);
                float m1 = maskRounded(p, _CornerRadius, _Margin + _Feather, _Feather);
                float borderRing = saturate(m0 - m1) * step(1e-5, abs(_Margin) + _Feather);

                float in0 = maskRounded(p, _CornerRadius, _InnerOffset, _InnerSoft);
                float in1 = maskRounded(p, _CornerRadius, _InnerOffset + _InnerWidth, _InnerSoft);
                float innerRing = saturate(in0 - in1) * step(1e-5, _InnerWidth);
                ...
                float2 b0 = float2(0.5 - _CornerRadius, 0.5 - _CornerRadius);
                float dEdge = sdRoundRect(p, b0, _CornerRadius);
                float x = -dEdge;
                float aa = _GlowSoft + fwidth(dEdge);
                float glow = saturate(smoothstep(0, aa, x) * (1.0 - smoothstep(_GlowWidth, _GlowWidth + aa, x)));
                ...
                float2 uvR = rot2(uvC / s, radians(_DigitRotation));
                float a2 = (1.0 - 2.0 * _DigitInset);
                float2 uvD = uvR * a2 + 0.5 + float2(_DigitOffsetX, _DigitOffsetY);
                float4 digit = SAMPLE_TEXTURE2D(_DigitTex, sampler_DigitTex, uvD);
                float digitMask = maskRounded(i.uv - 0.5, _DigitCorner, _DigitInset, _DigitSoft);
                float dmask = digit.a * saturate(_DigitOpacity) * _DigitTint.a * digitMask;
